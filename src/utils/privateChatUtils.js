...
export const fetchPrivateChatMembers = async (chatId) => {
  ...
  if (chatSnap.exists()) {
    ...
    const memberUids = chatSnap.data().members;
...
export const fetchPendingMembers = async (chatId) => {
  const chatRef = doc(db, "privateChats", chatId);
  const chatSnap = await getDoc(chatRef);
  ...
  if (chatSnap.exists()) {
    ...
    if (!pendingSnapshot.empty) {
      ...
      const pendingInvitationsData = pendingSnapshot.docs.map((inv) =>
        inv.data(),
      );

      //using colon brackets to destructure object
      const pendingMembers = await Promise.all(
        pendingInvitationsData.map(async ({ user }) => {
