    ...
    <div className="flex gap-8">
      {tabs.length > 0 ? (
        tabs.map((tab) => (
          ...
          <button
            key={tab.id}
            className={`font-dm rounded-full p-1 cursor-pointer w-full text-xs md:text-sm whitespace-nowrap ${selectedTab === tab.id ? 'active-tab bg-neutral' : 'text-neutral bg-accent'}`}
