...
/**
 * @param {*} videoId 
 * @returns list of active users from the last 5 minutes of messages in a chat
 */
export const getActiveUsers = async (videoId) => {
  try {
    ...
    querySnapshot.forEach((doc) => {
      ...
      if (messageTimestamp >= fiveMinutesAgo) { //a user is 'active' iff they have a message in the chat sent in the last 5 minutes
...
export const fetchYoutubeDetails = async (videoId) => {
  try {
    const response = await fetch(`https://www.googleapis.com/youtube/v3/videos?id=${videoId}&key=${process.env.YOUTUBE_API_KEY}&part=snippet`); //1 api cost
...
export const fetchYoutubeVideoNameFromUrl = async (url) => {
  if(url) {
    try {
      ...
      const response = await fetch(`https://www.googleapis.com/youtube/v3/videos?id=${videoId}&key=${process.env.YOUTUBE_API_KEY}&part=snippet`); //make a call to an endpoint of the youtube server, USING a url with our API Key attached to it.
      const data = await response.json();
      if (data.items && data.items.length > 0) {
